...
def redundant_ties_model2(N_init, sampleN):
    ...
    #print sum(np.multiply(AClist, keys))
    ...
    for u in range(0, cn): 
        for v in T2G[u]:
            ...
            if v>=u:
                ...
                density = int(np.ceil((0.1*n1*n2)))
                print density
                for i in range(min(density, n2-1, n1-1)):
                    n= reps_ordered_cliquewise[u][0:i]
                    Gclique.add_edge(clique_start[u]+reps_ordered_cliquewise[u][i+1], clique_start[v]+reps_ordered_cliquewise[v][i+1])
    ...
    plt.figure(2)
    nx.draw(Gclique,pos,node_color=colors,node_size=100,cmap=plt.cm.Purples)
    ...
    np.savetxt('AdjMatrices/RT/RT'+str(N_init)+'_'+str(sampleN)+'.txt', A, fmt='%10.f', delimiter='\t') 

    with open('StructuralProperties/ClusteringCoeff/RT/RT'+str(N_init)+'.txt', "a") as text_file:
        text_file.write(str(nx.average_clustering(Gclique)) + "\n")

    with open('StructuralProperties/AveShortestPath/RT/RT'+str(N_init)+'.txt', "a") as text_file:
        text_file.write(str(nx.average_shortest_path_length(Gclique)) + "\n")

    with open('StructuralProperties/Density/RT/RT'+str(N_init)+'.txt', "a") as text_file:
    ...
    with open('StructuralProperties/DegreeDist/RT/RT'+str(N_init)+'.txt', "a") as text_file:
...
#Gclique = redundant_ties_model2(N_init, sampleN)            
Gclique = redundant_ties_model2(50, 0)            
